...
async def get_recent_anomalies(db: AsyncSession, limit: int):
    rows = (await db.execute(text("""
        SELECT anomaly_id, anomaly_type, CAST(anomaly_score AS DECIMAL(5,4)) AS anomaly_score, description,
               CAST(detected_at AS CHAR) AS detected_at, reviewed
        FROM anomaly_detections
        ORDER BY detected_at DESC
        LIMIT :limit
    """), {"limit": limit})).fetchall()
    return [{"anomaly_id": r[0], "anomaly_type": r[1], "anomaly_score": float(r[2]),
...
async def get_anomaly_detail(db: AsyncSession, anomaly_id: int):
    ...
    if row:
        return {"anomaly_id": row[0], "anomaly_type": row[1], "anomaly_score": float(row[2]),
