...
async def get_district_crime_density(db: AsyncSession):
    rows = (await db.execute(text("""
        ...
        LEFT JOIN case_masters cm ON cm.police_station_id = u.unit_id
            AND cm.crime_registered_date >= CURRENT_DATE - INTERVAL '30 days'
...
async def get_hotspots(db: AsyncSession, district_id: int | None, category_id: int | None, days: int):
    query = f"""
        SELECT hotspot_id, latitude::float, longitude::float, hotspot_radius_meters,
               incident_count, risk_score::float
        FROM crime_hotspots
        WHERE computed_date >= CURRENT_DATE - INTERVAL '{days} days'
    """
    params: dict = {}
    if district_id:
    ...
    return [
        {"hotspot_id": r[0], "latitude": r[1], "longitude": r[2],
         "radius_meters": r[3], "incident_count": r[4], "risk_score": r[5]}
        for r in rows
    ]
...
async def get_cases_in_bounds(db: AsyncSession, north: float, south: float, east: float, west: float):
    rows = (await db.execute(text("""
        SELECT cm.case_master_id, cm.crime_no, cm.latitude::float, cm.longitude::float, ch.crime_group_name
    ...
    return [{"case_master_id": r[0], "crime_no": r[1], "latitude": r[2], "longitude": r[3], "crime_head": r[4]} for r in rows]
...
async def get_temporal_pattern(db: AsyncSession, district_id: int | None, category_id: int | None):
    ...
    hourly_rows = (await db.execute(text(f"""
        SELECT EXTRACT(HOUR FROM incident_from_date)::int AS hour, COUNT(*) AS count
        FROM case_masters cm
    ...
    weekly_rows = (await db.execute(text(f"""
        SELECT EXTRACT(DOW FROM crime_registered_date)::int AS dow, COUNT(*) AS count
        FROM case_masters cm
        JOIN units u ON cm.police_station_id = u.unit_id
